...
class HiddenBridge(nn.Module):
    ...
    def forward(self, h, c):
        ...
        ), f"Expected {2*self.num_layers} but got {num_layers_total}"
...
# ====================== Decoder with MultiHeadAttention + Residual ======================
class DecoderWithMHA(nn.Module):
    def __init__(
        self,
        vocab_size,
        embed_dim,
        ...
        self.enc_proj = nn.Linear(2 * enc_hidden_dim, dec_hidden_dim, bias=False)
    ...
    def forward(
        ...
        residual = dec_out + attn_out
        ln_out = self.layernorm(residual)
...
# ====================== Итоговая Seq2Seq ======================
class BiLSTMSeq2SeqMHA_Residual(nn.Module):
    def __init__(
        self,
        vocab_size,
        embed_dim,
    ...
    def forward(self, src_ids, tgt_ids, src_mask=None):
        ...
        logits, (_, _), _ = self.decoder(
            tgt_ids, dec_h, dec_c, encoder_outputs, src_mask=src_mask
        )
        return logits
